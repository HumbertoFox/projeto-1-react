...
if ($dados) {
    ...
    if ($crm_count > 0) {
        ...
        $query_checked_cpf = "SELECT COUNT(*) FROM doctors WHERE cpf = :cpf";
        ...
        if ($cpf_count > 0) {
            $response = [
                "error" => true,
                "message" => " CPF já Cadastrados!"
            ];
        } else {
            $query_checked_crm = "SELECT COUNT(*) FROM crm WHERE crm = :crm";
            $checked_crm = $conn->prepare($query_checked_crm);
            $checked_crm->bindParam(':crm', $dados['crm'], PDO::PARAM_STR);
            $checked_crm->execute();
            $crm_count = $checked_crm->fetchColumn();
            ...
            $query_add_doctor = "INSERT INTO doctors (crm, cpf, telephone, email, address_id, user_id) VALUES (:crm, :cpf, :telephone, :email, :address_id, :user_id)";
            ...
            $cad_doctor->bindParam(':email', $dados['email'], PDO::PARAM_STR);
