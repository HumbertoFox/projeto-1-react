...
if ($dados) {
    ...
    $query_get_address_id = "SELECT address_id FROM patients WHERE cpf = :cpf";
    $checked_cpf = $conn->prepare($query_checked_cpf);
    $checked_cpf->bindParam(':cpf', $dados['cpf'], PDO::PARAM_STR);
    $checked_cpf->execute();
    if ($query_get_address_id === "") {
    ...
    $query_addconsultation = "INSERT INTO consultation (cpf, crm_doctor, plan, value, observation, dataconsult, user_id) VALUES (:cpf, :crm_doctor, :plan, :value, :observation, :dataconsult, :user_id)";
};